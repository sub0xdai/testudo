...
## T16+T17 (2026-04-21) — Verification + Archival
`cargo clippy --all-targets` passes (warnings only — `useless_conversion` in cex_client.rs and `manual_contains` in evaluator.rs are pre-existing). `cargo test` result: 718 passed, 1 pre-existing ignored failure (`routes::auth::tests::test_me_returns_user_info`, confirmed pre-spec). `bun run build` (journal): clean pass.

The sole pre-existing test failure (`test_me_returns_user_info`) asserts a UUID field that comes back as JSON `Null`. This is an auth route test that predates this spec; do not fix unless the `/me` endpoint shape is explicitly in scope for a future auth spec.
