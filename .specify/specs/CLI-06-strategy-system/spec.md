...
**Spec ID:** CLI-06-strategy-system
**Date:** 2026-05-31
**Status:** Draft
**Class:** Feature / Integration
**Priority:** P0 — connects the Lean verification layer (STRAT-01) to the trading harness (CLI-04); without this bridge, proofs are inert math
**Depends on:** STRAT-01-lean-proofs, CLI-04-strategy-registry
**Series:** CLI-06 (Strategy System Bridge)

---

## Problem Statement

STRAT-01 produces verified Lean 4 proofs and TOML strategy artifacts. The `tudo` trading harness (CLI-01 through CLI-05) runs LLM-powered strategies. The two are disconnected:
...
│  └─────────────────────┘                                     │
│                                                              │
│  Consumed by: tudo harness → AgentState.strategy             │
└─────────────────────────────────────────────────────────────┘
```
...
```rust
// tudo/src/strategies/loader.rs — NEW
...
```rust
// tudo/src/strategies/loader.rs — NEW (continued)
...
```rust
// tudo/src/strategies/constraints.rs — NEW
...
```rust
// tudo/src/strategies/tools.rs — NEW
...
```rust
// tudo/src/strategies/validator.rs — NEW
...
All in `tudo/src/strategies/` (adds to the harness from CLI-04):
