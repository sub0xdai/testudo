# Agent setup for `testudo`

This is a deepsec scanning workspace. Project `testudo` was just registered
(target: `..`). Setup is incomplete — `data/testudo/INFO.md`
...
2. **Fill in `data/testudo/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id testudo
pnpm deepsec process --project-id testudo
```
