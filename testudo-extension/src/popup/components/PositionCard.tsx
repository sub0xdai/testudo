...
export default function PositionCard(props: PositionCardProps) {
  ...
  // KEEP: status badges use trading data colors
  const statusClass = () => {
  ...
  return (
    <div
      class={`bg-bg-panel border border-border-subtle p-4 card-depth ${isLong() ? "accent-long" : "accent-short"}`}
    ...
    >
      ...
      <div class="flex items-center justify-between mb-2.5">
        ...
        <span
          class={`text-[9px] uppercase tracking-wider px-2.5 py-0.5 font-sans font-medium ${statusClass()}`}
      ...
      {/* Row 2: Entry + SL — KEEP: SL is trading data */}
          ...
          <For each={props.trade.take_profit_targets}>
            {(tp, i) => (
              <div class="flex items-center gap-2" data-testid="position-tp">
                ...
                <span class={`text-[9px] font-sans font-medium px-1.5 py-px ${
      ...
      <div class="flex items-center justify-between mt-3 pt-2.5 border-t border-border-subtle">
        <div class="flex gap-2">
          <span
            class={`text-[9px] uppercase font-sans font-medium px-2 py-0.5 border ${beClass()}`}
            data-testid="position-be-badge"
          >
            {beLabel()}
          </span>
          <span
            class="text-[9px] uppercase font-sans font-medium px-2 py-0.5 border text-text-dim border-border-subtle"
